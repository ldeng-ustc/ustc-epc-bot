...
class EPCBot(threading.Thread):
    ...
    URL_BOOKABLE = [
        {"type": "Situational Dialogue",   "url": URL_ROOT +
            "m_practice.asp?second_id=2001"},
        {"type": "Topical Discussion",     "url": URL_ROOT +
            "m_practice.asp?second_id=2002"},
        {"type": "Debate",                 "url": URL_ROOT +
            "m_practice.asp?second_id=2003"},
        {"type": "Drama",                  "url": URL_ROOT +
            "m_practice.asp?second_id=2004"},
        {"type": "Pronunciation Practice", "url": URL_ROOT +
            "m_practice.asp?second_id=2007"}
    ]
    ...
    def get_bookable_epc(self):
        ...
        def foo(type_url: str, epc_name):
            ...
            for i in range(1, len(tr)):
                ...
                class_bookable = False
                for item in self.wday_filter:
                    if item["wday"] == wday_ and item["time"] == time_ and item["enable"] == True:
                        class_bookable = True

                if not class_bookable:
                    continue
        ...
        for epc_type in self.type_filter:
            ...
            task = threading.Thread(target=foo, args=(type_url, epc_type, ))
            task.start()
            tasks.append(task)
